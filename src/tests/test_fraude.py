...
from src.data.split_dataset import split_train_test, split_train_test_sample
...
class TestModel(unittest.TestCase):
    ...
    def _run_test_set(self, test_set, test_labels, tolerance=0.2):  # pragma: no cover
        """Executa conjunto de testes com tolerância de erro."""
        failures = 0
    ...
    def test_model_ninety_fraud(self):
        ...
        n_fraud = max(1, int(total_test * 0.1))
        n_normal = int(n_fraud * 9)
    ...
    def test_save_runtime_raises(self):
        ...
        self.assertIn("Nenhum modelo treinado", str(context.exception))
    ...
    def test_amount_standardized(self):
        """Verifica se a coluna Amount foi padronizada (média≈0, desvio≈1)."""
        df = load_dataset()
        mean = df["Amount"].mean()
        std = df["Amount"].std()
        tol = 0.1
    ...
    def test_evaluate_raises_without_threshold(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            model.evaluate()
        self.assertIn("Threshold não definido", str(context.exception))
